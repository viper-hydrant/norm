...
func create_instance(wg *sync.WaitGroup) (r *Instance, err error) {
	...
	if r.handle = C.NormCreateInstance(false); r.handle == C.NORM_INSTANCE_INVALID {
...
func (o *Instance) create_session(address string, port int, node_id Node_id) (sess *Session, err error) {
	...
	node_id_ := C.NormNodeId(node_id)
	if node_id_ == 0 {
		node_id_ = C.NormNodeId(uint32(time.Now().Unix()))
	}
...
func (o *Session) start_sender(id C.NormSessionId, buffer_space uint, segment_size, block_size, num_parity uint16, fec_id uint8) bool {
	return bool(C.NormStartSender(o.handle, id, C.UINT32(buffer_space), C.UINT16(segment_size), C.UINT16(block_size), C.UINT16(num_parity), C.UINT8(fec_id)))
}
...
func (o *Instance) do_event() error {
	...
	case Event_type_tx_object_purged:
		...
		o.sess().release(o.nevent.object)
